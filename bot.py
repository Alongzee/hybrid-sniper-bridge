...
Both @claude and @gemini route to Gemini 2.0 Flash (free tier: 1500 req/day)

Commands:
  @claude <message>  → replies via Gemini (Claude persona)
  @gemini <message>  → replies via Gemini
  /status            → shows bot is alive
  /help              → shows usage
"""
...
# ─── SAFE ERROR MESSAGE (never exposes keys or URLs) ─────────────────────────
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Route @claude → Gemini with Claude persona
    if lower.startswith("@claude"):
        ...
        try:
            ...
            logger.error(f"Claude route error: {type(e).__name__}")
            await update.message.reply_text(safe_error(e))

    # Route @gemini → Gemini with Gemini persona
        ...
        try:
            ...
            logger.error(f"Gemini route error: {type(e).__name__}")
            await update.message.reply_text(safe_error(e))
...
async def status_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        ...
        "• `@gemini <msg>` → Gemini\n"
        "• `/help` → usage guide\n\n"
        "_Powered by Gemini 2.0 Flash (free tier)_",
        parse_mode="Markdown",
    )
